  ...
  return (
    <div className="mx-auto flex max-w-[560px] flex-col gap-4 p-4 pb-[78px]">
      ...
      <Link href="/settings" className="flex items-center gap-3.5 rounded-2xl border border-bd bg-sf px-4 py-4">
        ...
        <span className="text-[13px] font-semibold leading-none text-teal">Edit</span>
      </Link>
      ...
      <button
        onClick={logout}
        className="flex items-center justify-center gap-2 rounded-2xl border border-bd bg-sf py-4 text-[15px] font-semibold leading-none text-out"
      >
...
function Tile({ icon: Icon }: { icon: LucideIcon }) {
  return (
    <span className="flex size-9 flex-none items-center justify-center rounded-[10px] bg-tlsf text-teal">
...
function Row({ icon, label, sub, href }: { icon: LucideIcon; label: string; sub?: string; href: string }) {
  return (
    <Link href={href} className="flex items-center gap-3.5 border-b border-hr2 px-4 py-4 last:border-b-0 active:bg-bg2">
      <Tile icon={icon} />
      <div className="min-w-0 flex-1">
        <div className="text-[15px] font-bold leading-none text-ink">{label}</div>
...
function ThemeRow() {
  return (
    ...
    >
      <Tile icon={Sun} />
      <div className="min-w-0 flex-1">
        <div className="text-[15px] font-bold leading-none text-ink">Theme</div>
        <div className="mt-1.5 text-xs font-medium leading-none text-fnt">Light or dark appearance</div>
      </div>
      <span className="text-[15px] font-semibold leading-none text-teal">Toggle</span>
    </button>
  );
}
