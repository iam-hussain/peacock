...
export function Share({ data }: { data: ShareData }) {
  ...
  return (
    <div className="mx-auto max-w-[1200px] p-4 pb-[78px] md:p-[26px] md:pb-[26px]">
      ...
      <div className="grid items-start gap-[22px] lg:grid-cols-[330px_1fr]">
        ...
        <aside className="flex flex-col gap-[18px] rounded-[18px] border border-bd bg-sf p-5 lg:sticky lg:top-7">
          <div>
            ...
            <div className="flex gap-[3px] rounded-[11px] border border-hair bg-bg2 p-1">
              {(["club", "member"] as const).map((m) => (
                <button key={m} onClick={() => setMode(m)} className={`flex-1 rounded-lg py-2.5 text-center text-[13px] font-semibold leading-none transition-colors ${mode === m ? "bg-sf text-ink shadow-[0_1px_2px_var(--shadow)]" : "text-mut"}`}>
          ...
          {mode === "club" ? (
              ...
              <div>
                <div className="grid grid-cols-2 gap-2">
                  {SECTION_KEYS.map((k) => {
                    const on = sections[k];
                    return (
                      <button key={k} onClick={() => setSections((s) => ({ ...s, [k]: !s[k] }))} className={`flex items-center gap-2.5 rounded-[11px] border px-[13px] py-3 ${on ? "border-teal/50 bg-tlsf" : "border-bd2 bg-sf"}`}>
                        ...
                        <span className={`text-[13px] font-semibold leading-none ${on ? "text-teal" : "text-ink"}`}>{SECTION_LABELS[k]}</span>
                      </button>
                    );
                  })}
                </div>
                <div className="mt-[9px] text-[11.5px] font-medium leading-[1.4] text-fnt">{sectionCount} section{sectionCount === 1 ? "" : "s"} — combined into one image</div>
              </div>
              <div>
                <Label>Include</Label>
            ...
            <>
              <div className="relative">
                <Label>Member</Label>
                <button onClick={() => setPickerOpen((v) => !v)} className="flex w-full items-center gap-3 rounded-[11px] border border-bd bg-sf px-[13px] py-3 text-left">
                  {selected && <span className="flex size-9 flex-none items-center justify-center rounded-full bg-bg2 text-[12px] font-bold leading-none text-mut">{initials(selected.name)}</span>}
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-[15px] font-bold leading-none text-ink">{selected?.name ?? "Select a member"}</span>
                    <span className="mt-1.5 block text-[11px] font-medium leading-none text-fnt">{selId === data.defaultMemberId ? "Defaults to you · click to change" : "click to change"}</span>
                  </span>
                  <ChevronDown className={`size-4 flex-none text-fnt transition-transform ${pickerOpen ? "rotate-180" : ""}`} strokeWidth={2} />
                </button>
                {pickerOpen && (
                  <div className="absolute z-20 mt-1.5 flex max-h-[300px] w-full flex-col gap-[5px] overflow-auto rounded-[12px] border border-bd bg-sf p-1.5 shadow-[0_10px_30px_var(--shadow)]">
                    {pickerMembers.map((m) => {
                      const on = m.id === selId;
                      return (
                        <button key={m.id} onClick={() => { setSelId(m.id); setPickerOpen(false); }} className={`flex items-center gap-[11px] rounded-[10px] px-[11px] py-[9px] text-left ${on ? "bg-tlsf" : "hover:bg-bg"}`}>
                          <span className="flex size-8 flex-none items-center justify-center rounded-full bg-bg2 text-[11px] font-bold leading-none text-mut">{initials(m.name)}</span>
                          <span className="min-w-0 flex-1">
          ...
          <div className="flex flex-col gap-[9px] border-t border-hr2 pt-4">
            <button onClick={doShare} disabled={!!busy || stage !== "ready"} className="flex w-full items-center justify-center gap-[9px] rounded-xl bg-teal p-3.5 text-sm font-bold leading-none text-white shadow-[0_8px_18px_rgba(14,140,130,0.24)] disabled:opacity-60">
              <Upload className="size-4" strokeWidth={2} /> {busy === "sh" ? "Rendering…" : "Share image"}
            </button>
            <button onClick={doDownload} disabled={!!busy || stage !== "ready"} className="flex w-full items-center justify-center gap-[9px] rounded-xl border border-bd2 bg-sf p-[13px] text-sm font-bold leading-none text-ink disabled:opacity-60">
              <Download className="size-4" strokeWidth={2} /> {busy === "dl" ? "Rendering…" : "Download PNG"}
            </button>
            <div className="mt-0.5 text-center text-[10.5px] font-medium leading-[1.4] text-fnt">PNG · high-resolution · {mode === "club" ? sectionNote || "nothing selected" : "member statement"}</div>
          </div>
        </aside>

        {/* ---------- preview stage ---------- */}
        <main className="relative min-h-[200px] overflow-hidden rounded-[18px] border border-bd2 bg-bg2 md:min-h-[560px]">
          ...
          {stage === "ready" && (
            <div className="pk-stage relative flex max-h-[200px] justify-center overflow-auto p-7 md:max-h-[78vh]">
      ...
      {toast && <div className="fixed bottom-7 left-1/2 z-[60] -translate-x-1/2 rounded-xl bg-ink px-5 py-3.5 text-[13px] font-semibold leading-none text-white shadow-[0_14px_34px_rgba(20,32,30,0.32)]">{toast}</div>}
    </div>
  );
}
...
function ToggleRow({ label, on, onClick }: { label: string; on: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} className="flex w-full items-center justify-between rounded-[11px] border border-bd bg-sf px-[13px] py-3 text-left">
      <span className="text-[13px] font-semibold leading-none text-ink">{label}</span>
...
function StageMsg({ icon: Icon, spin, tone, title, sub, children }: { icon?: typeof Users; spin?: boolean; tone?: "out"; title: string; sub: string; children?: React.ReactNode }) {
  return (
    <div className="relative flex min-h-[200px] flex-col items-center justify-center gap-3.5 px-8 text-center md:min-h-[560px]">
      {spin && <Loader2 className="size-[34px] animate-spin text-teal" strokeWidth={2.5} />}
      {Icon && <span className={`flex size-[52px] items-center justify-center rounded-full ${tone === "out" ? "bg-outbg text-out" : "bg-tlsf text-teal"}`}><Icon className="size-6" strokeWidth={2} /></span>}
